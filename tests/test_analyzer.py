...
class AnalyzerTests(unittest.TestCase):
    ...
    def test_application_cpe_case_and_whitespace_are_deduplicated(self) -> None:
        scan = Scan(source="test.xml", hosts=(Host(
            address="192.0.2.10", status="up", ports=(Port(
                8080, "tcp", "open", "http", cpes=(
                    "CPE:/A:Example:Web:1.0",
                    " cpe:/a:example:web:1.0 ",
                ),
            ),),
        ),))
        ...
        self.assertEqual(len(application_findings), 1)
        self.assertEqual(
            application_findings[0].evidence.lower().count("cpe:/a:example:web:1.0"),
            1,
        )
    ...
    def test_application_cpe_protocol_case_is_deduplicated(self) -> None:
        scan = Scan(source="test.xml", hosts=(Host(
            address="192.0.2.10", status="up", ports=(
                Port(8080, "TCP", "open", "http", cpes=("cpe:/a:example:web:1.0",)),
                Port(8080, "tcp", "open", "http", cpes=("cpe:/a:example:web:1.0",)),
            ),
        ),))
